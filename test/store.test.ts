...
test("saveGraph refuses a programmatic overlong term without writing a poisoned cache", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "myco-index-overlong-"));
  try {
    const graph = new SearchGraph();
    graph.setFile(
      "a.txt",
